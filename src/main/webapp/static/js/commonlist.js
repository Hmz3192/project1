...
var Pagination = {
    ...
    Create: function(e) {
        ...
        var html = [
            '<a class="glyphicon glyphicon-chevron-left" style="font-size:24px;">&lsaquo;</a>', // previous button
            '<span></span>',  // pagination container
            '<a class="glyphicon glyphicon-chevron-right" style="font-size:24px;">&rsaquo;</a>'  // next button
        ];
    ...
    Init: function(e, data) {
        ...
        Pagination.Create(e);
        Pagination.Start();
        $(".glyphicon-chevron-left").bind("click", function() { //减
			var thisNum = $(".thisPage").text() - 1
			if (thisNum <= 1) {
				thisNum = 1;
			}
			$(".thisPage").text(thisNum)
		})
		$(".glyphicon-chevron-right").bind("click", function() { //加
			var thisNum = parseInt($(".thisPage").text()) + 1
			if (thisNum >= $(".totlePage").text()) {
				thisNum = $(".totlePage").text();
			}
			$(".thisPage").text(thisNum)
		})
    }
...
function init(number,selected) {
    ...
    Pagination.Init(document.getElementById('pageSmall'), {
        size: number , // pages size
        page: selected,  // selected page
        step: 0  // pages before and after current
    });
